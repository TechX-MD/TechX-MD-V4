...
// 1. Web UI Homepage (Loads instantly without loading Baileys)
app.get('/', (req, res) => {
...
// 2. Pair Endpoint - Lazy Loads Baileys ONLY when requested!
app.get('/pair', async (req, res) => {
    ...
    try {
        ...
        if (!res.headersSent) {
            return res.status(500).send({ error: "Failed to generate pairing code. Try again." });
        }
    }
});
